...
class Test(unittest.TestCase):
    ...
    @unittest.skip('skip')
    @patch('main.api._Api._send', new=get_privat_response, )
    ...
    def test_xml_api(self):
        response = requests.get('http://localhost:5000/api/xrates/xml')
        self.assertIn('<xrates>', response.text)
        xml_rates = xmltodict.parse(response.text)
        self.assertIsInstance(xml_rates['xrates']['xrate'], list)
        self.assertEqual(len(xml_rates['xrates']['xrate']), 5)

    def test_json_api(self):
        response = requests.get('http://localhost:5000/api/xrates/json')
        json_rates = response.json()
    ...
    def test_json_api_uah(self):
        response = requests.get('http://localhost:5000/api/xrates/json?to_currency=980')
        json_rates = response.json()
        self.assertIsInstance(json_rates, list)
